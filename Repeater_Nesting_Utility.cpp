...
int main() {
  std::string num_files, num_repetitions, create_filename, archive_7zip, build_string;
  char command[128];
  ...
  if (archive_7zip == "Y") {
    ...
    snprintf(command, 128, "7z a -mx=1 NEST-FULLPOWER.ZIP NEST-0.TXT >> logfile.txt");
    system(command);
    ...
    for (filenum = 1; filenum <= stoi(num_files); filenum++) {
      ...
      if (archive_7zip == "Y") {
        ...
        snprintf(command, 128,
                 "7z a -mx=1 NEST-FULLPOWER.ZIP NEST-%d.TXT >> logfile.txt", filenum);
        system(command);
