...
int main() {
  ...
  long long int repnum, filenum;
  std::fstream myfile;

  cout << "Intention Repeater Nesting File Creation Utility v3.1." << endl;
  ...
  if (myfile.is_open()) {
    for (repnum = 1; repnum <= stoll(num_repetitions); repnum++) {
  ...
  for (filenum = 1; filenum <= stoll(num_files); filenum++) {
